...
echo -e "Releasing libbwt-jni v$version\n"
...
sed -i -r "s~libbwt-jni[0-9a-z.-]+-x86_64-linux\.~libbwt-jni-$version-x86_64-linux.~g; s~/(download|tag)/v[0-9a-z.-]+~/\1/v$version~;" README.md
...
if [ -z "$SKIP_BUILD" ]; then
  ...
  docker_run() {
    docker run -it --rm -u $(id -u) -v $(pwd):/usr/src/libbwt-jni \
      -v ${CARGO_HOME:-$HOME/.cargo}:/usr/local/cargo \
      -v ${SCCACHE_DIR:-$HOME/.cache/sccache}:/usr/local/sccache \
      -v ${GRADLE_USER_HOME:-$HOME/.gradle}:/usr/local/gradle \
      -w /usr/src/libbwt-jni --entrypoint scripts/$2 $1
  }
  docker_run bwt-builder          build.sh
  docker_run bwt-builder-osx      build.sh
  docker_run bwt-builder-android  build-android.sh

  rm -r dist/*/ # remove subdirectories, keep files only
fi
